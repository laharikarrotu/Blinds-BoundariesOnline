...
try:
    import torch
    from app.u2net_inference import U2NetWindowDetector
    ML_AVAILABLE = True
...
def detect_window(image_id: str = Query(..., description="The image_id returned from /upload-image")):
    if not ML_AVAILABLE:
        return JSONResponse(
            status_code=503, 
            content={
                "error": "ML model not available. Running on free tier without PyTorch.",
                "suggestion": "Upgrade to paid tier for full ML functionality"
            }
        )
    ...
    if mask_url:
        return {"message": "Window mask generated successfully!", "mask_url": mask_url}
    else:
        return JSONResponse(status_code=500, content={"error": "Failed to upload mask to Azure Blob Storage."})
...
def try_on(
    ...
    if result_url:
        return {"message": "Try-on result generated successfully!", "result_url": result_url}
    else:
        return JSONResponse(status_code=500, content={"error": "Failed to upload try-on result to Azure Blob Storage."}) 