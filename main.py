...
try:
    ...
    # Set environment variables to handle OpenCV issues (fix libGL.so.1 error on Azure)
    ...
    try:
        ...
        import traceback
        traceback.print_exc()
        # Fallback to old main_hybrid (if available in legacy folder)
        try:
            # Try importing from legacy directory
            import sys
            legacy_path = os.path.join(current_dir, 'legacy')
            if legacy_path not in sys.path:
                sys.path.insert(0, legacy_path)
            from main_hybrid import app as hybrid_app
            print("✅ Successfully imported main_hybrid.py from legacy (fallback)")
            application = hybrid_app
        except ImportError as e1:
            print(f"⚠️ Could not import main_hybrid.py from legacy: {e1}")
